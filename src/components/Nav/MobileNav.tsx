  ...
  return (
    <div {...rest}>
      <div className="md:hidden">
        <Popover>
          <PopoverTrigger className="cursor-pointer text-lg font-semibold text-gray-50 hover:text-gray-100  p-1 rounded-full bg-gray-300/70 hover:bg-gray-300 transition-all">
            <Image alt="menu" src={menuImg} width={50} height={50} />
          </PopoverTrigger>
            ...
            {navItems.map((val) => {
              return (
                <Link
                  ...
                  href={val.url}
                >
                  <Image src={val.Icon} alt={val.title + "Icon"} width={40} height={40} />
                  <p> {val.title}</p>
                </Link>
              );
            })}
